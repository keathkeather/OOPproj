...
public class TransactionLog extends JPanel {
    ...
    public TransactionLog(int customerID) {
        setSize(1080, 720);
        ...
        NavBar navbar = new NavBar(this, customerID); // Pass "this" instead of "panel" for the constructor of NavBar
        add(navbar, BorderLayout.WEST);
        add(scrollPane);

        // Fetch and display the transaction log
        fetchTransactionLog(customerID); // Pass customerID to the fetchTransactionLog method

        setVisible(true);
    }

    private void fetchTransactionLog(int customerID) {
        ...
        try {
            ...
            String sql = "SELECT tl.transactionDate, tl.transactionAmount, tl.TransactionDesc, at.accountTypeDesc " +
                    ...
                    "WHERE tl.customerID = ?"; // Use a placeholder for customerID

            statement = connection.prepareStatement(sql);
            statement.setInt(1, customerID); // Set the value for the customerID placeholder
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            // Create an instance of TransactionLog with a customerID
            TransactionLog transactionLog = new TransactionLog(2);
            JFrame frame = new JFrame("Transaction Log");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.add(transactionLog);
            frame.pack();
